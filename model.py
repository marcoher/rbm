...
class rbm(object):
  ...
  """
  def __init__(self, visible_dim, hidden_dim, visible_type = "binary", seed = 0, name = "rbm"):
    """
    ...
    assert visible_dim>0,"number of visible units must be positive: %s" % visible_dim
    assert hidden_dim>0,"number of hidden units must be positive: %s" % hidden_dim
    assert visible_type.strip().lower() in implemented,"type of visible units is not implemented: %s" % visible_type
  ...
  def fit(self, data, max_epochs = 1000, learning_rate = 0.1, cd_steps = 1, verbosity = 10):
    """
    Fit the model to a dataset
    data          : an array of shape [num_samples, visible_dim]
  ...
  def _units(self):
    ...
    """
    if self.visible_type in ["binary", "multinomial"]:

      self.visible_dtype = tf.uint8

    elif self.visible_type in ["gaussian"]:
  ...
  def _conditional_v_sample(self, h):
    ...
    h : a tensor of shape [:, hidden_dim]
    """
  ...
  def _conditional_h_sample(self, v):
    ...
    v : a tensor of shape [:, visible_dim]
    """
...
#    summary_writer = tf.train.SummaryWriter(path + '/logs', graph_def = self.sess.graph_def)
